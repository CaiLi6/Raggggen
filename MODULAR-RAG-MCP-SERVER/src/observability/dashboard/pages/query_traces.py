...
def _render_evaluate_button(trace: Dict[str, Any], idx: int) -> None:
    ...
    # Use stable per-trace key to prevent session-state collisions when
    # query list order changes after new traces are appended.
    trace_state_id = str(trace.get("trace_id") or f"idx_{idx}")
    ...
    answer_key = f"eval_answer_{trace_state_id}"
    ...
    with col_btn:
        clicked = st.button(
            "📏 Ragas Evaluate",
            key=f"eval_trace_{trace_state_id}",
    ...
    result_key = f"eval_result_{trace_state_id}"

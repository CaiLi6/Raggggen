...
class QueryKnowledgeHubTool:
    ...
    @staticmethod
    def _is_rerank_disabled_by_env() -> bool:
        """Allow temporary rerank disable for troubleshooting latency bottlenecks."""
        raw = os.getenv("RAG_DISABLE_RERANK", "").strip().lower()
    ...
    async def execute(
        ...
        rerank_enabled = self.config.enable_rerank and not self._is_rerank_disabled_by_env()
        ...
        search_timeout_s = self._get_int_env("RAG_SEARCH_TIMEOUT_SECONDS", 120)
        rerank_timeout_s = self._get_int_env("RAG_RERANK_TIMEOUT_SECONDS", 30)
        overall_timeout_s = self._get_int_env("RAG_OVERALL_TIMEOUT_SECONDS", 180)

        def _stage_timeout(configured_seconds: int) -> float:
            elapsed = time.monotonic() - total_t0
            remaining = max(1.0, overall_timeout_s - elapsed)
            return float(min(configured_seconds, int(remaining)))
        ...
        try:
            ...
            await asyncio.wait_for(
                asyncio.to_thread(self._ensure_initialized, effective_collection),
                timeout=_stage_timeout(init_timeout_s),
            )
            ...
            logger.info("[query_knowledge_hub] stage=initialization elapsed_ms=%.1f", _init_elapsed)
            if _init_elapsed > 5000:
                logger.warning(
                    "[query_knowledge_hub] slow stage detected: initialization elapsed_ms=%.1f",
                    _init_elapsed,
                )
            ...
            _search_elapsed = (time.monotonic() - _search_t0) * 1000.0
            trace.record_stage(
                "hybrid_search_total",
                {
            ...
            logger.info(
                "[query_knowledge_hub] stage=hybrid_search_total elapsed_ms=%.1f result_count=%s",
                _search_elapsed,
                len(results),
            )
            if _search_elapsed > 15000:
                logger.warning(
                    "[query_knowledge_hub] slow stage detected: hybrid_search_total elapsed_ms=%.1f",
                    _search_elapsed,
                )
            ...
            if rerank_enabled and results:
                ...
                try:
                    results = await asyncio.wait_for(
                        ...
                        timeout=_stage_timeout(rerank_timeout_s),
                    )
                except TimeoutError:
                    logger.warning(
                        "[query_knowledge_hub] rerank timeout after %ss, falling back to original order",
                        rerank_timeout_s,
                    )
                    results = results[:effective_top_k]
                _rerank_elapsed = (time.monotonic() - _rerank_t0) * 1000.0
                trace.record_stage(
                    "rerank_total",
                    {
                        "enabled": True,
                ...
                logger.info(
                    "[query_knowledge_hub] stage=rerank_total elapsed_ms=%.1f result_count=%s",
                    _rerank_elapsed,
                    len(results),
                )
                if _rerank_elapsed > 10000:
                    logger.warning(
                        "[query_knowledge_hub] slow stage detected: rerank_total elapsed_ms=%.1f",
                        _rerank_elapsed,
                    )
            elif results:
                trace.record_stage(
                    ...
                    {
                        "enabled": False,
                        "reason": "disabled_by_env" if self._is_rerank_disabled_by_env() else "config_disabled",
                        "result_count": len(results),
                    },
                    elapsed_ms=0.0,
                )
                logger.info("[query_knowledge_hub] stage=rerank_total skipped enabled=False")
            ...
            _build_elapsed = (time.monotonic() - _build_t0) * 1000.0
            trace.record_stage(
                "response_build",
                {
                ...
                elapsed_ms=_build_elapsed,
            )
            logger.info("[query_knowledge_hub] stage=response_build elapsed_ms=%.1f", _build_elapsed)
            ...
            total_elapsed = (time.monotonic() - total_t0) * 1000.0
            trace.metadata["total_elapsed_ms"] = round(total_elapsed, 1)
            logger.info("[query_knowledge_hub] total_elapsed_ms=%.1f", total_elapsed)
            if total_elapsed > 30000:
                logger.warning(
                    "[query_knowledge_hub] overall slow query detected: total_elapsed_ms=%.1f",
                    total_elapsed,
                )
            ...
            logger.exception("query_knowledge_hub timeout: %s", e)
            trace.record_stage(
                "timeout",
                {
                ...
                elapsed_ms=(time.monotonic() - total_t0) * 1000.0,
            )
            TraceCollector().collect(trace)
            return self._build_error_response(
                query,
                effective_collection,
                f"服务端查询超时（overall={overall_timeout_s}s）",
            )

...
|---------|---------|-------------|---------|
| `query_knowledge_hub` | 主检索入口，支持可插拔检索模式（Classic RAG / Graph-RAG / Hybrid+Graph）并可选 Rerank | `query: string`, `top_k?: int`, `collection?: string`, `retrieval_mode?: "hybrid"|"graph"|"hybrid_graph"`, `graph_hops?: 1..3` | 返回带引用的结构化结果，并记录检索模式用于追踪可视化 |
...
- **图检索（Graph）**：基于实体重叠与邻域扩展信号进行图式召回（轻量 Graph-RAG 路径）。
- **混合图检索（Hybrid + Graph）**：Dense + Sparse + Graph 三路并行召回，再统一融合。
- **混合召回 + 精排（Hybrid + Rerank）**：在混合召回基础上，增加精排步骤进一步提升相关性。

本项目当前采用 **可插拔检索模式 + 可选精排** 策略：
...
- **图召回（Graph Route）**：使用 `GraphRetriever` 基于实体重叠与可选 hops 邻域信号召回候选。
- **融合（Fusion）**：使用加权 RRF (Reciprocal Rank Fusion) 对多路结果统一排序，支持 Dense/Sparse/Graph 权重配置。
- **精排（Rerank）**：对融合后的候选集进行重排序，支持 None / Cross-Encoder / LLM Rerank 三种模式。

> **当前实现说明**：目前系统已支持 `hybrid`、`graph`、`hybrid_graph` 三种运行模式，并通过 `query_knowledge_hub` 的可选参数按请求粒度切换；默认仍保持 Classic Hybrid RAG 行为，确保向后兼容。
...
- **配置文件结构示例** (`config/settings.yaml`)：
  ...
  graph:
    enabled: false               # false 时保持现有 Classic Hybrid RAG
    mode: "hybrid"              # hybrid | graph | hybrid_graph
    top_k: 20
    hops: 1
...
- **双链路全覆盖追踪 (Dual-Pipeline Tracing)**：
  ...
  - **Query Trace**：以 `trace_id` 为核心，记录一次查询从 Query 输入到 Response 输出的全过程（query_processing → dense/sparse/graph（按模式可选）→ fusion（可选）→ rerank（可选）），包含各阶段候选数量、分数分布与耗时。
...
- `user_query`：用户原始查询
- `collection`：检索的知识库集合
- `retrieval_mode`：本次检索模式（`hybrid`/`graph`/`hybrid_graph`）
- `graph_hops`：图检索扩展深度（可选）
...
| **Graph Retrieval（可选）** | 图检索候选、hops、命中实体信息、耗时 |
| **Fusion（可选）** | 融合后的统一排名、algorithm、retrieval_mode、weights、耗时 |
...
- **组件配置卡片**：读取 `Settings`，展示当前可插拔组件的配置状态：
  ...
  - Graph RAG：enabled/mode/top_k/hops/weight_in_fusion
...
- **模式分布统计**：按 Query Trace 汇总 `hybrid` / `graph` / `hybrid_graph` 调用次数，直观看到运行占比。
...
- **单次查询详情**：
  - **模式标签**：每条请求展示 `retrieval_mode`（hybrid / graph / hybrid_graph）与 graph_hops。
  - **耗时瀑布图**：展示 query_processing/dense/sparse/graph/fusion/rerank 各阶段时间分布（按模式动态展示）。
  - **Dense / Sparse / Graph 对比**：并列展示各路召回结果的 Top-N 文档 ID 与分数。
...
- Query Traces 页面基于 `retrieval_mode`、`graph_hops`、`graph_retrieval` 阶段动态展示，能够区分 Classic RAG 与 Graph-RAG。
- Fusion 详情页展示 `weights` 与 `retrieval_mode`，用于解释三路融合的排序来源。
...
│   │   │   ├── hybrid_search.py         # 可插拔检索编排 (hybrid / graph / hybrid_graph)
│   │   │   ├── dense_retriever.py       # 稠密向量检索
│   │   │   ├── sparse_retriever.py      # 稀疏检索 (BM25)
│   │   │   ├── graph_retriever.py       # 图检索 (实体重叠 + 邻域扩展)
...
| `hybrid_search.py` | 可插拔检索编排 | 支持 `hybrid` / `graph` / `hybrid_graph`；并行多路召回与加权 RRF 融合 |
...
| `graph_retriever.py` | 图检索 | 实体重叠召回 + 可选 hops 邻域扩展，输出可解释命中信息 |
...
本系统由旧版单机学习型 RAG 项目全面重构而来。下表概括关键架构升维：

| 维度 | 旧版 (DEV_SPEC) | 新版 (Enterprise) |
|------|-----------------|-------------------|
| **存储引擎** | SQLite + Chroma（嵌入式） | PostgreSQL 15+ / Milvus Standalone / Redis 7 |
| **LLM 推理** | Azure OpenAI / DeepSeek 云端 API | LM Studio 本地多模型调度: 主推理模型 (如 35B 多模态) + 轻量路由模型 (如 1-3B)，均通过 OpenAI 兼容 API |
| **Embedding** | OpenAI API / Ollama（进程内） | BGE-M3 常驻 FastAPI 微服务 |
| **Rerank** | CrossEncoder（进程内） | bce-reranker-base_v1 常驻 FastAPI 微服务 |
| **任务调度** | 同步阻塞 Pipeline | Celery + Redis 异步分布式任务队列 |
| **并发能力** | 单用户 MCP Client | 20 人 SSO/OIDC 鉴权 + GPTCache 语义缓存 + 限流 |
| **检索策略** | 单一 Hybrid Search (Dense+BM25+RRF) | **Agentic RAG: ReAct Agent + Hybrid/Graph 多路召回** |
| **知识图谱** | 无 | **Kùzu 嵌入式图数据库 — 芯片/参数/替代料/勘误四元知识图谱** |
| **智能编排** | 无 | **ReAct/Tool Calling Agent Orchestrator (LLM 驱动动态工具调用)** |
| **文档解析** | MarkItDown (纯文本) | 三级表格提取 (pdfplumber / Camelot / PaddleOCR) |
| **接口层** | MCP Stdio Server | FastAPI REST/WebSocket + Gradio/Vue3 前端 |
| **用户系统** | 无 | SSO/OIDC (Keycloak/钉钉/飞书) + RBAC 权限模型 |
| **部署方式** | Docker Compose 单机 | Helm Charts + K8s 云原生集群 |
| **容错机制** | 无 | CircuitBreaker + Tenacity 指数退避重试 |
| **向量数据库** | Milvus Standalone 单机 | Milvus Cluster / Qdrant 分布式集群 (百万级) |
| **CI/CD** | 手动部署 | GitHub Actions 四阶段流水线 (SonarQube + Trivy + 滚动更新) |

## 1.3 保留的设计遗产

以下旧版设计原则经验证后在新架构中完整保留：
...
6. **Pipeline 阶段解耦** — 每个 Pipeline 阶段独立可测、可监控、可恢复。v3.0 中原有 Pipeline 重构为 Agent Tools，保持单一职责的同时获得 LLM 驱动的动态编排能力。
